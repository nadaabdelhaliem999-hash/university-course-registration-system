...
bool authenticateUser(const vector<User>& users, string_view email, string_view password) {
    return any_of(users.begin(), users.end(),
...
bool registerForCourse(vector<User>& users, string_view email,
                       vector<Course>& courses, int courseId) {
...
bool dropCourse(vector<User>& users, string_view email,
                vector<Course>& courses, int courseId) {
    auto userIt = find_if(users.begin(), users.end(),
                          [&email](const User& u) { return u.email == email; });
    if (userIt == users.end()) {
    ...
    auto idIt = find(userIt->enrolledCourseIds.begin(),
                     userIt->enrolledCourseIds.end(), courseId);
...
void viewSchedule(const vector<User>& users, string_view email,
...
void handleRegisterForCourse(vector<User>& users, string_view loggedInEmail,
...
void handleDropCourse(vector<User>& users, string_view loggedInEmail,
                      vector<Course>& courses) {
...
void handleViewSchedule(const vector<User>& users, string_view loggedInEmail,
                        const vector<Course>& courses) {
    viewSchedule(users, loggedInEmail, courses);
}
